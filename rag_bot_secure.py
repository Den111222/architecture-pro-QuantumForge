...
from security import ProductionSecurityFilter
...
ТВОЙ ОТВЕТ (обязательно в формате 1. Анализ вопроса: ... 2. Поиск в контексте: ... 3. Вывод: ... 4. Ответ: ...):"""
    ...
    def ask(self, query: str, user_id: str = "anonymous") -> Dict[str, Any]:
        start = datetime.now()

        # Валидация запроса через security.py
        is_safe, reason, confidence = self.security.validate_query(query, user_id)
        if not is_safe:
            return {
                'query': query,
                'answer': f"⚠️ Запрос отклонён: {reason}",
                'sources': [],
                'found': False,
                'blocked': True,
        ...
        # Санитизация ответа через security.py
        answer = self.security.sanitize_response(answer, query, user_id)
    ...
    def interactive_mode(self):
        ...
        while True:
            ...
            if q.lower() == '/sources':
                ...
                continue
            if q.lower() == '/stats':
                print(f"\nСтатистика: {self.index.ntotal} чанков")
                continue

            resp = self.ask(q)
            print(f"\n{resp['answer']}")
            if show_sources and resp['sources']:
                print(f"\nИсточники: {', '.join(resp['sources'])}")
...
def main():
    ...
    if args.interactive:
        ...
        print("python rag_bot_secure.py --interactive")
